...
> 当前可用能力以 [assembly_workflow.md](./assembly_workflow.md) 为准。已实现
> `kind=assembly`、多实体合法性校验、按 solids 聚合指标、标准件库渐进查询、装配 Skill
> 路由和无显示环境分色 PNG；跨包 commit pin、自动 BOM、约束/干涉求解仍是本设计稿中的
> 后续项。模型不得把后续接口当成已存在的工具。
